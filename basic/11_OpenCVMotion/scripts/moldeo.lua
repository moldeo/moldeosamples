...
function AddParam( this, moldeo_label, param_name )
	
	paramlbl = moldeo_label..":"..param_name

	this.moscript_objects_params[paramlbl] = -1  --undefined
	this.moscript_objects_params[paramlbl] = this:GetObjectParamIndex( GetObject( this, moldeo_label ), param_name )
	if (this.moscript_objects_params[paramlbl]>=0) then
		return this.moscript_objects_params[paramlbl]
	end

	Debug( this, "moldeo.lua error: AddMember("..moldeo_label..","..param_name..")")
...
function GetParam( this, moldeo_label, param_name )

	paramlbl = moldeo_label..":"..param_name
	if ( this.moscript_objects_params[paramlbl]~=nil) then
		if this.moscript_objects_params[paramlbl]<=0 then			
			return -1
		else
			return this.moscript_objects_params[paramlbl]
		...
		this:PushDebugString("moldeo.lua > GetParam : Error : paramlbl"..paramlbl)
	end
end
...
function SetMemberValue( this, moldeo_label, member_name, member_value )
	...
	if (idobject>=0 and idmember>=0) then
		...
		this:PushDebugString("moldeo.lua: SetMemberValue: ERROR! label:"..moldeo_label.." or param:"..member_name.." does not exist" )
	end
end

function Enable( this, moldeo_label )
	idobject = GetObject( this, moldeo_label )
	if (idobject>=0) then
		this:ObjectEnable( idobject, idmember, member_value )
	else
		this:PushDebugString("moldeo.lua: Enable: ERROR! object label:"..moldeo_label.." does not exists" )
	end
end

function Disable( this, moldeo_label )
	idobject = GetObject( this, moldeo_label )
	if (idobject>=0) then
		this:ObjectDisable( idobject, idmember, member_value )
	else
		this:PushDebugString("moldeo.lua: Disable: ERROR! object label:"..moldeo_label.." does not exists" )
	end
end

function SetParamValue( this, moldeo_label, param_name, value_index )

	idobject = GetObject( this, moldeo_label )
	idparam = GetParam( this, moldeo_label, param_name)
	if (idobject>=0 and idparam>=0) then
		this:SetObjectCurrentValue( idobject, idparam, value_index )
	else
		this:PushDebugString("moldeo.lua: SetParamValue: ERROR! label:"..moldeo_label.." or param:"..param_name.." does not exist" )
	end
end
...
function GetTimerActualState(this )
	stateobj = GetActualState(this)
	estado = ActualStateString(this)
	if (stateobj~=nil) then
		return stateobj.timer_time
	else
		this:PushDebugString("moldeo.lua::GetTimerActualState > ERROR: state "..estado.." doesnt exists.")
	end
...
-----------------------------------------------------------------------------------------
-- MOLDEO LUA only XmlParser based on XmlParse from Alexander Makeev
-----------------------------------------------------------------------------------------
XmlParser = {};
