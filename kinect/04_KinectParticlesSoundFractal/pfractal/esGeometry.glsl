...
void main() {
  ...
  //float depth = clamp( floor( 64.0 * (6.0 - vertex[0].position.z ) / 12.0 ), 0.0, 63.0 );
  float depth = 0;
  depth = clamp( floor( 64.0 * (2.0 - vertex[0].position.z ) / 4.0 ), 0.0, 63.0 );
  //depth = vertex[0].position.z
  //if (vertex[0].position.z<=-0.1)  { depth = 0.0; }
  //else if (vertex[0].position.z>=0.1) { depth = 2.0; }
  //else { depth = 1.0; }
  //gl_Layer = int(depth);
  gl_Layer = int(depth);
  EmitVertex();
