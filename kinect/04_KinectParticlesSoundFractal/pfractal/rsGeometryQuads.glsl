...
void main() {
 ...
 vec4 position4 = vertex[0].position;
 vec4 position5 = vertex[0].position;
 ...
 vec2 vertex_coord4 = vertex_coord0;
 vec2 vertex_coord5 = vertex_coord0;
...
/*
 position0 = vertex[0].position + mOri * vec4( -sx, sy, 0.0, 0.0 );
 position1 = vertex[0].position + mOri * vec4( -sx, -sy, 0.0, 0.0 );
 position2 = vertex[0].position + mOri * vec4( sx, sy, 0.0, 0.0 );
 position3 = vertex[0].position + mOri * vec4( sx, -sy, 0.0, 0.0 );
*/
bool valid_index;
float nti=1.0,ntj=1.0;
//sphere or tube
//if (emittertype==1 || emittertype==2) {
if (1==1) {
  valid_index = 0<=ti && ti<(fcols[0]) && 0<=tj && tj<(frows[0]-1);
  if (ti==(fcols[0]-1)) nti = -ti;
  //if (tj==(frows[0]-1)) ntj = -ti;
} else valid_index = 0<ti && ti<(fcols[0]-1) && 0<tj && tj<(frows[0]-1);
 ...
 if (valid_index) {
   ...
   fti = (ti-1.0) / fcols[0];
   ftj = (tj) / frows[0];
   position4 = vec4( texture2D( t_position, vec2( fti, ftj )).xyz, 1.0 );

   fti = (ti) / fcols[0];
   ftj = (tj-1.0) / frows[0];
   position5 = vec4( texture2D( t_position, vec2( fti, ftj )).xyz, 1.0 );
...
float tfti = (1.0) / fcols[0];
float tftj = (1.0) / frows[0];

//vec3 v0normal = texture2D( t_normal, vec2( fti, ftj )).xyz;
vec3 v0normal = texture2D( t_normal, vec2( fti, ftj )).xyz;
vec3 v1normal = texture2D( t_normal, vec2( fti, ftj+tftj )).xyz;
vec3 v2normal = texture2D( t_normal, vec2( fti+tfti, ftj )).xyz;
vec3 v3normal = texture2D( t_normal, vec2( fti+tfti, ftj+tftj )).xyz;
