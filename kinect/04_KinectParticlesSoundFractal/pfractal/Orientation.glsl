...
uniform sampler2D src_tex_unit3;// orientation
...
vec4 SetParticleOrientation( vec4 father_orien ) {
  vec4 orien3d;
  float alpha;
  float phi;
  float radius;

	vec4 me = texture2D( src_tex_unit4,  gl_TexCoord[0].st );
	vec4 al = texture2D( src_tex_unit5,  gl_TexCoord[0].st );
	vec4 vl = texture2D( src_tex_unit6,  gl_TexCoord[0].st );
	vec4 cf = texture2D( src_tex_unit7,  gl_TexCoord[0].st );

  if (emittertype==5) {

  ///TRACKER > use for KINECT texture
    orien3d = vec4 ( father_orien.x*al.r,
                   father_orien.y*al.g,
                   father_orien.z*al.b,
                   0.83 );
...
void main(void)
{
    ...
    vec4 src_scale = texture2D(src_tex_unit2, gl_TexCoord[0].st);
		vec4 src_orien = texture2D(src_tex_unit3, gl_TexCoord[0].st);
    vec4 father_orien = texture2D(src_tex_unit3, father_tcoord.st);
    ...
    vec2 TCoordMedium = vec2( (src_position.x-6.0)/12.0, -(src_position.y-6.0)/12.0 );
    ...
    if (State.age >= incremento_madurez ) {
        ...
        if (  State.age==incremento_madurez ) {
          if (State.origin==LEFTSTATE) {
            gl_FragColor = SetParticleOrientation( father_orien );
    ...
    } else gl_FragColor = vec4( src_orien.x, src_orien.y, src_orien.z, 0.0);
